...
class CamDataNode(Node):
    ...
    def __init__(self):
        ...
        self.ats.registerCallback(self.img_callback)

    def img_callback(self, data1, data2):
        ...
        print("callback image1 mean:", cv_image1.mean())
        print("callback image2 mean:", cv_image2.mean())

        combined_image = cv2.hconcat([cv_image1, cv_image2])

        combined_image_msg = bridge.cv2_to_imgmsg(
            combined_image,
            encoding='bgr8'
        )

        self.rgb_pub.publish(combined_image_msg)


rclpy.init(args=None)

cam_node = CamDataNode()
spin_thread = Thread(
    target=rclpy.spin,
    args=(cam_node,),
    daemon=True
...
class LoadStreams:
    ...
    def __init__(self, sources="file.streams", img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            # cv_image
            while combined_image is None and rclpy.ok():
            	time.sleep(0.1)
            
            cap = combined_image.copy()            
    ...
    def update(self, i, cap, stream):
        ...
        while rclpy.ok():
            if combined_image is not None:
                self.imgs[i] = combined_image.copy()
