...
class SegCoordPublisher(Node):
    def __init__(self):
        super().__init__("seg_coord_publisher")
    ...
    def publish_coords(self, coords):
        ...
        if coords is None or len(coords) == 0:
            ...
            msg.data = [int(x) for x in np.array(coords).flatten().tolist()]
...
def run(
    ...
    seg_pub_node = SegCoordPublisher()
            ...
            if len(det):
                ...
                annotator.masks(
                    ...
                    im_gpu=torch.as_tensor(
                        im0,
                        dtype=torch.float16
                    ).to(device).permute(2, 0, 1).flip(0).contiguous() / 255
                    if retina_masks
                    else im[i],
                )
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    ...
                    if int(cls) == 0 and conf >= 0.25:
                        if j < len(all_segments):
                            polygon_points = all_segments[j]
                            segment_mid = np.mean(polygon_points, axis=0)
                            rounded_segment_mid = np.round(segment_mid).astype(int)

                            current_seg_coords.extend(rounded_segment_mid.tolist())

                if len(current_seg_coords) > 0:
                    seg_pub_node.publish_coords(current_seg_coords)
                    print("Published seg_coords:", current_seg_coords)
                else:
                    seg_pub_node.publish_coords([])
                    print("No person segmentation")
...
def parse_opt():
    ...
    parser.add_argument("--line-thickness", default=3, type=int, help="bbox thickness")
    ...
    parser.add_argument("--retina-masks", action="store_true", help="plot masks in native resolution")
